...
def get_attn_subsequence_mask(seq):
    # Decoder部分中的mask self attention
    """建议打印出来看看是什么的输出（一目了然）
...
def get_attn_pad_mask(seq_q, seq_k):
    # pad mask的作用：在对value向量加权平均的时候，可以让pad对应的alpha_ij=0，这样注意力就不会考虑到pad (P) 向量
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, input_Q, input_K, input_V, attn_mask):
        ...
        """
        residual, batch_size = input_Q, input_Q.size(0) # residual保留原来的输入
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.layers = nn.ModuleList([EncoderLayer() for _ in range(n_layers)])  # 几个Encoder Block集合到一起
    ...
    def forward(self, enc_inputs):
        ...
        enc_self_attn_mask = get_attn_pad_mask(enc_inputs, enc_inputs)  # [batch_size, src_len, src_len] # 去掉P在句中的意义
...
class Transformer(nn.Module):
    def __init__(self):
        ...
        self.encoder = Encoder().to(device)  # Encoder
        self.decoder = Decoder().to(device)  # Decoder
        self.projection = nn.Linear(d_model, tgt_vocab_size, bias=False).to(device)  # Linear
